...
import (
	...
	actionModels "github.com/red-gold/telar-web/src/models/actions"
	userModels "github.com/red-gold/telar-web/src/models/users"
...
func createToken(model *TokenModel) (string, error) {
	...
	privateKeydata, err := ioutil.ReadFile(*coreConfig.PrivateKeyPath)
	if err != nil {
		log.Fatalf("private key, unable to read path: %s, error: %s", *coreConfig.PrivateKeyPath, err.Error())
	}
...
func initUserSetup(userId uuid.UUID, email string, avatar string, displayName string, role string) error {
	...
	settingModel := userModels.CreateSettingGroupModel{
		Type: "notification",
		List: []userModels.SettingGroupItemModel{
			userModels.SettingGroupItemModel{
