...
import (
	...
	ac "github.com/red-gold/telar-web/src/controllers/users/auth/config"
	"github.com/red-gold/telar-web/src/controllers/users/auth/provider"
	dto "github.com/red-gold/telar-web/src/domain/users"
	model "github.com/red-gold/telar-web/src/models/users"
...
func SignupTokenHandle(db interface{}) func(http.ResponseWriter, *http.Request, server.Request) (handler.Response, error) {
	return func(w http.ResponseWriter, r *http.Request, req server.Request) (handler.Response, error) {
		...
		model := &model.SignupTokenModel{
			User: model.UserSignupTokenModel{
		...
		// Verify Captha
		keydata, err := ioutil.ReadFile(*config.RecaptchaKeyPath)

		if err != nil {
			fmt.Printf("Unable to read path %s error: %s", *config.RecaptchaKeyPath, err)
			return handler.Response{StatusCode: http.StatusInternalServerError, Body: []byte("{error: 'Error happened in verifying captcha!'}")},
				err
		}
		recaptcha := utils.NewRecaptha(string(keydata))
		remoteIpAddress := utils.GetIPAdress(r)
		recaptchaStatus, recaptchaErr := recaptcha.VerifyCaptch(model.Recaptcha, remoteIpAddress)
		if recaptchaErr != nil {
			fmt.Printf("Unable to read path %s error: %s", *config.RecaptchaKeyPath, recaptchaErr)
			return handler.Response{StatusCode: http.StatusInternalServerError, Body: []byte("{error: 'Error happened in verifying captcha!'}")},
				recaptchaErr
		}
		if !recaptchaStatus {
