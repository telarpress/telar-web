...
func ForgetPasswordFormHandler(db interface{}) func(http.ResponseWriter, *http.Request, server.Request) (handler.Response, error) {
	...
	return func(w http.ResponseWriter, r *http.Request, req server.Request) (handler.Response, error) {
		...
		// Send email
		emailRefPassword, errReadRef := ioutil.ReadFile(*appConfig.RefEmailPassPath)
		if errReadRef != nil {
			fmt.Printf("Unable to read path %s error: %s", *appConfig.RefEmailPassPath, errReadRef.Error())
			return handler.Response{StatusCode: http.StatusInternalServerError, Body: utils.MarshalError("unableToReadPath", "Unable to read email ref path!")},
				nil

		}
		email := utils.NewEmail(*appConfig.RefEmail, string(emailRefPassword), *appConfig.SmtpEmail)
		...
		emailResStatus, emailResErr := email.SendEmail(emailReq, "html_template/email_link_verify_reset_pass.html", struct {
			...
			AppName: *appConfig.AppName,
			Link:    fmt.Sprintf("%s%s/password/reset/%s", *appConfig.WebDomain, prettyURL, verifyId),
