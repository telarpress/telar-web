...
import (
	...
	"github.com/red-gold/telar-web/src/constants"
...
func (s UserVerificationServiceImpl) CreateEmailVerficationToken(input EmailVerificationToken,
	...
	// Send email
	emailRefPassword, errReadRef := ioutil.ReadFile(*coreConfig.RefEmailPassPath)
	if errReadRef != nil {
		return "", fmt.Errorf("Unable to read path %s error: %s", *coreConfig.RefEmailPassPath, errReadRef)

	}
	email := utils.NewEmail(*coreConfig.RefEmail, string(emailRefPassword), *coreConfig.SmtpEmail)
	...
	// Generate JWT token
	privateKey, err := ioutil.ReadFile(*coreConfig.PrivateKeyPath)

	if err != nil {
		return "", fmt.Errorf("Unable to read path %s error: %s", *coreConfig.RefEmailPassPath, err)

	}
	return utils.GenerateJWTToken(privateKey, utils.TokenClaims{
		Claim: metaToken,
	}, 1)
}
...
func (s UserVerificationServiceImpl) CreatePhoneVerficationToken(input PhoneVerificationToken,
	...
	// Send SMS
	phoneAuthToken, err := ioutil.ReadFile(*coreConfig.PhoneAuthTokenPath)
	if err != nil {
		return "", fmt.Errorf("Unable to read path %s error: %s", *coreConfig.RefEmailPassPath, err)

	}
	phoneAuthId, err := ioutil.ReadFile(*coreConfig.PhoneAuthIdPath)
	if err != nil {
		return "", fmt.Errorf("Unable to read path %s error: %s", *coreConfig.RefEmailPassPath, err)

	}
	phone, phoneErr := utils.NewPhone(string(phoneAuthToken), string(phoneAuthId), *config.AppConfig.PhoneSourceNumber)
	if phoneErr != nil {
		return "", phoneErr
	}
	code := utils.GenerateDigits(4)
